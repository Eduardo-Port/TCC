...
async function getManageProduct(categoria = null) {
    let urlRequest = url;  
    if (categoria) {
        urlRequest = `${url}?categoria=${categoria}`;
    }
    ...
    if (!Array.isArray(data) || data.length === 0) {
        ...
        const flex = document.querySelector('#wrap-flex')
        p.innerText = "Nenhum produto encontrado com esta categoria";
